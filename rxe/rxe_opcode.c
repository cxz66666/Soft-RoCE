...
struct rxe_opcode_info rxe_opcode[RXE_NUM_OPCODE] = {
	...
	[IB_OPCODE_RC_SEND_ONLY] = {
		...
		.mask = RXE_PAYLOAD_MASK | RXE_REQ_MASK | RXE_COMP_MASK
				| RXE_RWR_MASK | RXE_SEND_MASK
				| RXE_START_MASK | RXE_END_MASK,
		.length = RXE_BTH_BYTES + DCQCN_ECN_SIZE,
		.offset = {
			[RXE_BTH] = 0,
			[RXE_DCQCN_ECN] = RXE_BTH_BYTES,
			[RXE_PAYLOAD] = RXE_BTH_BYTES + DCQCN_ECN_SIZE,
		}
	},
	[IB_OPCODE_RC_SEND_ONLY_WITH_IMMEDIATE] = {
		.name = "IB_OPCODE_RC_SEND_ONLY_WITH_IMMEDIATE",
		.mask = RXE_IMMDT_MASK | RXE_PAYLOAD_MASK | RXE_REQ_MASK
				| RXE_COMP_MASK | RXE_RWR_MASK | RXE_SEND_MASK
				| RXE_START_MASK | RXE_END_MASK,
		.length = RXE_BTH_BYTES + RXE_IMMDT_BYTES + DCQCN_ECN_SIZE,
		.offset = {
			[RXE_BTH] = 0,
			[RXE_IMMDT] = RXE_BTH_BYTES,
			[RXE_DCQCN_ECN] = RXE_BTH_BYTES
						+ RXE_IMMDT_BYTES,
			[RXE_PAYLOAD] = RXE_BTH_BYTES
						+ RXE_IMMDT_BYTES + DCQCN_ECN_SIZE,
		}
	},
	[IB_OPCODE_RC_ACKNOWLEDGE] = {
		.name = "IB_OPCODE_RC_ACKNOWLEDGE",
		.mask = RXE_AETH_MASK | RXE_ACK_MASK | RXE_START_MASK
				| RXE_END_MASK,
		.length = RXE_BTH_BYTES + RXE_AETH_BYTES + DCQCN_ECN_SIZE,
		.offset = {
			[RXE_BTH] = 0,
			[RXE_AETH] = RXE_BTH_BYTES,
