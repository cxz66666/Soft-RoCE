...
static inline void dcqcn_recv_pkt(struct rxe_qp *qp, struct rxe_pkt_info *pkt, struct rxe_pkt_info *ack_pkt) {
    ...
    if (qp->dcqcn_byte_count >= DCQCN_BC_EXPIRES) {
        ...
        if (qp->dcqcn_t < 5 && qp->dcqcn_bc < 5) {
            qp->dcqcn_rate = (qp->dcqcn_rt + qp->dcqcn_rate) / 2;
        } else {
            qp->dcqcn_rt += 5;
            qp->dcqcn_rate = (qp->dcqcn_rt + qp->dcqcn_rate) / 2;
        }
    ...
    // TODO
    if (*(uint32_t *)(pkt->hdr + rxe_opcode[pkt->opcode].offset[RXE_DCQCN_ECN]) == 1 && rdtsc() - qp->dcqcn_now_time >= 50 * dcqcn_rdtsc_mul) {
        *(uint32_t *)(ack_pkt->hdr + rxe_opcode[ack_pkt->opcode].offset[RXE_DCQCN_ECN]) = 1;
        qp->dcqcn_now_time = rdtsc();
    } else {
        *(uint32_t *)(ack_pkt->hdr + rxe_opcode[ack_pkt->opcode].offset[RXE_DCQCN_ECN]) = 0;
    }
}
...
static inline void dcqcn_recv_ack(struct rxe_qp *qp, struct rxe_pkt_info *pkt) {
    ...
    if (*(uint32_t *)(pkt->hdr + rxe_opcode[pkt->opcode].offset[RXE_DCQCN_ECN]) == 1) {
        qp->dcqcn_rt = qp->dcqcn_rate;
        qp->dcqcn_rate = qp->dcqcn_rt - qp->dcqcn_A * qp->dcqcn_rt / 256 / 2;
        qp->dcqcn_A = (256 * qp->dcqcn_A - (qp->dcqcn_A * DCQCN_G) + DCQCN_G) / 256;
        qp->dcqcn_byte_count = 0;
    ...
    if (rdtsc() - qp->dcqcn_last_time >= 55 * dcqcn_rdtsc_mul) {
        qp->dcqcn_A = (256 * qp->dcqcn_A - (qp->dcqcn_A * DCQCN_G)) / 256;
        qp->dcqcn_last_time = rdtsc();
        qp->dcqcn_t++;
        if (qp->dcqcn_t < 5 && qp->dcqcn_bc < 5) {
            qp->dcqcn_rate = (qp->dcqcn_rt + qp->dcqcn_rate) / 2;
